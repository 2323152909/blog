...
#### 你订阅了我的公众号
我这个微信公号作者是发布者，您这些微信用户是订阅者「我发送一篇文章的时候，关注了【前端程序员成长指北】的订阅者们都可以收到文章。
...
officeAccounts.subscribe('前端程序员成长指北',xiaoming.readArticle);
officeAccounts.subscribe('前端程序员成长指北',xiaogang.readArticle);
officeAccounts.subscribe('某公众号',xiaoming.readArticle);

officeAccounts.unSubscribe('某公众号',xiaoming.readArticle);

officeAccounts.publish('前端程序员成长指北','前端程序员成长指北的Node文章');
...
```javascript
小明收到的 前端程序员成长指北的Node文章
小刚收到的 前端程序员成长指北的Node文章
```
- 结论
